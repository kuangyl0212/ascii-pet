...
class TestBackupSubmenuLabels:
    """验证恢复子菜单标签格式：[自动/手动] YYYY-MM-DD HH:MM:SS"""
    ...
    def test_show_tray_menu_submenu_label_format(self, pet_window):
        ...
        def fake_append_menu(menu, flags, item_id, label):
            if label and isinstance(label, str) and (label.startswith('[自动]') or label.startswith('[手动]')):
        ...
        assert appended_labels[0] == '[自动] 2026-03-10 14:05:30'
        assert appended_labels[1] == '[手动] 2026-03-11 09:00:15'
    ...
    def test_show_context_menu_submenu_label_format(self, pet_window):
        ...
        assert len(appended_labels) == 2
        assert appended_labels[0] == '[自动] 2026-05-20 08:30:45'
        assert appended_labels[1] == '[手动] 2026-05-21 16:00:00'
